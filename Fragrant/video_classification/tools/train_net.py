...
def train_epoch(
    ...
    for cur_iter, (inputs1, inputs2, labels1, labels2, _, _) in enumerate(train_loader):
        # Transfer the data to the current GPU device.
        if cfg.NUM_GPUS:
            if isinstance(inputs1, (list,)):
                for i in range(len(inputs1)):
                    inputs1[i] = inputs2[i].cuda(non_blocking=True)
            else:
        ...
        if cfg.MIXUP.ENABLE:
            samples1, labels1 = mixup_fn(inputs1[0], labels1)
            inputs1[0] = samples1
            samples2, labels2 = mixup_fn(inputs2[0], labels2)
            inputs2[0] = samples2
